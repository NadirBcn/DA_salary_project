    ...
    while len(jobs) < num_jobs:  #If true, should be still looking for new jobs.
        ...
        for job_button in job_buttons:  
            ...
            while not collected_successfully:
                try:
#                     company_name = driver.find_element_by_xpath('.//div[@class="css-xuk5ye"]').text
                    company_name = driver.find_element_by_class_name('css-xuk5ye').text
#                     location = driver.find_element_by_xpath('.//div[@class="css-56kyx5"]').text
                    location = driver.find_element_by_class_name('css-56kyx5').text
#                     job_title = driver.find_element_by_xpath('.//div[contains(@class, "css-1j389vi")]').text
                    job_title = driver.find_element_by_class_name('css-1j389vi').text
...
#                 pass
            ...
            try:
#                 salary_estimate = driver.find_element_by_xpath('.//span[@class="css-0 e1wijj242"]').text
                salary_estimate = driver.find_element_by_class_name('css-1hbqxax').text
            except NoSuchElementException:
                salary_estimate = -1 #You need to set a "not found value. It's important."

            try:
#                 rating = driver.find_element_by_xpath('.//span[@class="css-1m5m32b e1tk4kwz4"]').text
                rating = driver.find_element_by_class_name('css-1m5m32b').text
            except NoSuchElementException:
...
#                driver.find_element_by_xpath('.//div[@class="tab" and @data-tab-type="overview"]').click()
                driver.find_element_by_xpath("//div[@data-item = 'tab']").click()
                ...
                try:
                    #size = driver.find_element_by_xpath('.//div[@class="EmpBasicInfo"]//label[text()="Size"]//following-sibling::*').text
                    #size = driver.find_element_by_xpath("//*[@id='EmpBasicInfo']").text THIS WORKS A BIT 
                ...
                try:
                    #founded = driver.find_element_by_xpath('.//div[@id="EmpBasicInfo"]//label[text()="Founded"]//following-sibling::*').text
                    #founded = driver.find_element_by_xpath("//i[@class='css-1pldt9b.e1pvx6aw1']/following-sibling::span[text()='Founded']").text
